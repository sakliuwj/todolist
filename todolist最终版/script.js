...
function renderTodoItemList(todoItems, finishedItems) {
    ...
    for (let i=0; i < todoItems.length; i++ ) {
        ...
        titleEl.addEventListener("mousedown",(e)=>{
            let mark=0//如果mark=0说明没有被执行过下面的函数，也就是用户还没改动。
            let originEl =todoItems[i].title
            titleEl.addEventListener("input",(e)=>{
                if(mark==0){ 
                  titleEl.addEventListener("mouseleave",(e)=>{
                      if(originEl!=titleEl.innerText){ //不相等说明用户改动了。
                         if(mark==0 && confirm("确认修改？")){
                                todoItems[i].title = titleEl.innerText;
                                mark=1;  //说明函数confirm被执行过，防止一直询问
                                renderTodoItemList(todoItems, finishedItems);
            
                                originEl=titleEl.innerText
                            }else{
                                todoItems[i].title=originEl; 
...
function renderInputPane(todoItems) {
    ...
    hisBtnEl.addEventListener("click", (e)=>{
        if (hisBtnEl.classList.contains("open")) {
            ...
            renderTodoItemList(todoItems, finishedItems)//这行也是测试的吧可能(x)运行一遍生成结果
        } else {
